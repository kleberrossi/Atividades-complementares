...
def admin_processar_requisicao(req_id):
    conn = get_db_connection()
    requisicao = conn.execute("SELECT * FROM requisicoes WHERE id = ?", (req_id,)).fetchone()
    if not requisicao:
        flash("Requisição não encontrada.", "error")
        return redirect(url_for("admin_requisicoes"))
...
def admin_adicionar_atividade():
    if request.method == "POST":
        ...
        conn = get_db_connection()
        try:
            conn.execute("INSERT INTO atividades (grupo, nome, limite_horas, tipo_atividade) VALUES (?, ?, ?, ?)", (grupo, nome, limite_horas, tipo_atividade))
            conn.commit()
...
def admin_editar_atividade(atividade_id):
    ...
    if request.method == "POST":
        ...
        tipo_atividade = request.form["tipo_atividade"]
        try:
            conn.execute("UPDATE atividades SET grupo = ?, nome = ?, limite_horas = ?, tipo_atividade = ? WHERE id = ?", (grupo, nome, limite_horas, tipo_atividade, atividade_id))
            conn.commit()
